...
class ChimeraDataset(Dataset):
    ...
    def __getitem__(self, idx):
        c = self.samples[idx]
        L = c.lengths['total']
        t5 = c.lengths['trna_5end']
        t3 = c.lengths['trna_3end']

        m = {'A':0,'C':1,'G':2,'U':3,'T':3}
        oh = np.zeros((L,4), dtype=np.float32)
        for i,ch in enumerate(c.sequence[:L]):
            if ch.upper() in m:
                oh[i, m[ch.upper()]] = 1.0

        cmap = c.contact_map[:L,:L].copy().astype(np.float32)
        domain = c.domain_labels[:L].copy()

        return (torch.tensor(oh), torch.tensor(cmap), torch.tensor(domain, dtype=torch.long),
                t5, t3)
...
def chimera_collate(batch):
    ...
    return seqs, labels, masks, t5s, t3s
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        for step, (seqs, labels, masks, t5s, t3s) in enumerate(loader):
            seqs = seqs.to(device)
