...
from src.dataset import MultiFileDatasetUpgrade, collate_pad_v2
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for step, (seqs, bppms, mfe_contacts, labels, masks) in enumerate(train_loader):
            seqs = seqs.to(args.device)
            bppms = bppms.to(args.device)
            mfe_contacts = mfe_contacts.to(args.device)
            labels = labels.to(args.device)
            masks = masks.to(args.device)
            B, L, _ = seqs.shape

            # 前向 (bpRNA 模式: 域标签全 0)
            logits = model(seqs, bppm=bppms, mfe_contact=mfe_contacts, mask=masks,
        ...
        with torch.no_grad():
            for seqs, bppms, mfe_contacts, labels, masks in val_loader:
                seqs = seqs.to(args.device)
                bppms = bppms.to(args.device)
                mfe_contacts = mfe_contacts.to(args.device)
                labels = labels.to(args.device)
