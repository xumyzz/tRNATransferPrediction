...
# ═══════════════════════════════════════════════════════════════════
# SpotRNA_LSTM_Refined_BPPM_Chimeric (v2 — 域 one-hot 注入)
#
# 核心改动（AlphaFold 式）:
#   1. MultiheadAttention 之前拼接 2 维域 one-hot
#   2. Linear 投影回原维度
#   3. 去掉跨域 attention mask → 模型自己学
#   4. bpRNA 训练时域恒为 [1,0]，chimera 推理时动态变化
# ═══════════════════════════════════════════════════════════════════
...
class SpotRNA_LSTM_Refined_BPPM_Chimeric(nn.Module):
    def __init__(self, config):
        ...
        self.num_res1d = config.RESNET_LAYERS
        self.lstm_hidden = getattr(config, 'LSTM_HIDDEN', self.hidden_dim)
        self.attn_dim = self.lstm_hidden * 2
        ...
        self.attention = nn.MultiheadAttention(
            embed_dim=self.attn_dim, num_heads=4, batch_first=True
        )

        self.seg_pos_emb = SegmentRelativePosEmb(dim=self.attn_dim, max_len=1000)

        # 【新增】域 one-hot 投影: (attn_dim + 2) → attn_dim
        self.domain_proj = nn.Linear(self.attn_dim + 2, self.attn_dim)

        dim_1d = self.hidden_dim + self.attn_dim + self.attn_dim
        dim_2d_input = dim_1d * 2
    ...
    def forward(self, x, bppm=None, mask=None,
        ...
        # ── 5. 分段位置编码 ──
        seg_pos = self.seg_pos_emb(seg_ids)
        x_lstm_pos = x_lstm + seg_pos                                   # (B, L, attn_dim)

        # ═══════════════════════════════════════════════════════════
        # 【AlphaFold 式改动】
        # 6. 域 one-hot 注入 + 投影回原维度
        # ===========================================================
        # bpRNA 预训练: seg_ids 全 0 → feat 恒为 [1, 0]
        # chimera 推理: seg_ids 含 0/2 → feat 动态变化
        # 模型自己学会「域内自由关注，跨域自己调控」— 不需要 mask 禁止
        # ═══════════════════════════════════════════════════════════

        dom_feat = torch.zeros(B, L, 2, device=device, dtype=x_lstm_pos.dtype)
        if seg_ids is not None:
            is_ins = (seg_ids == 2)
            dom_feat[:, :, 0] = (~is_ins).float()   # 域 0 (tRNA)
            dom_feat[:, :, 1] = is_ins.float()       # 域 2 (insert)
        else:
            dom_feat[:, :, 0] = 1.0                   # 全在域 0

        x_lstm_pos = torch.cat([x_lstm_pos, dom_feat], dim=-1)   # (B, L, attn_dim+2)
        x_lstm_pos = self.domain_proj(x_lstm_pos)                 # (B, L, attn_dim)

        # 提取 padding mask（仅对真实 padding，不涉及域）─────
        if mask is not None:
            if mask.dim() == 4:
                mask_1d = mask[:, 0, 0, :]
            elif mask.dim() == 2:
        ...
        # ═══════════════════════════════════════════════════════════
        # Attention: 无跨域 mask，只保留 padding mask
        # 域信息已在特征里，MHA 自己学何时互相关注
        # ═══════════════════════════════════════════════════════════
        ...
        # ── 12. 跨域 2D 硬掩码（仅输出层，不涉及 attention）──
