...
Adapted for the new TRNA_Mamba_Refined architecture.
...
# ====== 修改 1: 导入全新的 Mamba 模型 ======
# 请确保你的 TRNA_Mamba_Refined 类保存在 src/model.py 或对应路径下
try:
    from src.model import TRNA_Mamba_Refined
except ImportError:
    print("Error: 找不到 TRNA_Mamba_Refined 模型，请检查导入路径！")
    sys.exit(1)
...
def train_model(args):
    ...
    # ====== 修改 2: 实例化新的 TRNA_Mamba_Refined 模型 ======
    model = TRNA_Mamba_Refined(config).to(args.device)
...
def main():
    ...
    # Model arguments (仍然兼容新模型中的 1D ResNet 和 LSTM 参��)
    parser.add_argument('--resnet_layers', type=int, default=8,
                        help='Number of 1D ResNet layers')
    ...
    # UFold postprocessing args
    parser.add_argument('--pp_offset', type=float, default=0.5,
                        help='UFold postprocess offset threshold (on probabilities)')
    parser.add_argument('--pp_min_loop', type=int, default=4,
                        help='UFold postprocess min loop length: disallow |i-j| < this')
            ...
            if device_str.startswith('cuda'):
                ...
                if ':' in device_str:
                    ...
                    if len(parts) != 2 or not parts[1].isdigit():
                        raise ValueError(
                            f"Invalid CUDA device format: '{device_str}'. Expected 'cuda' or 'cuda:N'"
                        )
                    device_idx = int(parts[1])
                    if device_idx >= torch.cuda.device_count():
                        raise ValueError(
                            f"CUDA device {device_idx} not available (only {torch.cuda.device_count()} devices)"
                        )
