...
def is_trna_entry(entry: RNAEntry,
                  ...
                  len_range: Tuple[int, int] = None,
                  use_length_fallback: bool = False) -> bool:
    """
    判断一条 RNA 是否为 tRNA。
    
    判断优先级:
      1. 名称关键词匹配
      2. (回退) 长度在 tRNA 典型范围 (60-95nt) 内
      3. (回退) 结构特征: 配对碱基数 >= 15 (三叶草至少3个茎)
    
    Args:
        entry: RNA 条目
        keywords: 自定义关键词列表
        len_range: 自定义长度范围 (min, max)
        use_length_fallback: 是否启用长度+结构回退（关键词匹配不到时使用）
    ...
    # 方法2: 长度+结构回退（仅在关键词完全匹配不到时启用）
    if use_length_fallback:
        L = entry.length
        if len_range[0] <= L <= len_range[1]:
            # 辅助: 结构验证 (tRNA 应有足够配对数，三叶草至少 3 个茎 ≈ 15 bp)
            pairs = dotbracket_to_pairs(entry.structure)
            if len(pairs) >= 15:
                return True
...
def is_trna_by_length_and_structure(
    entry: RNAEntry,
    min_len: int = 60,
    max_len: int = 95,
    min_pairs: int = 15,
) -> bool:
    """纯粹基于长度和配对数的 tRNA 判断（用于关键词匹配失败的回退）"""
    if not (min_len <= entry.length <= max_len):
        return False
    pairs = dotbracket_to_pairs(entry.structure)
...
def classify_entries(entries: List[RNAEntry],
                     trna_keyword: str = 'tRNA',
                     trna_min_len: int = 60,
                     trna_max_len: int = 95) -> Tuple[List[RNAEntry], List[RNAEntry]]:
    """
    将条目分为 tRNA 和非 tRNA 两组。
    
    策略:
      1. 先用关键词匹配
      2. 如果关键词匹配的 tRNA < 5 条，自动切换为 长度+结构 回退模式
    
    Args:
        entries: RNA 条目列表
        trna_keyword: tRNA 识别关键词 (逗号分隔)
        trna_min_len: tRNA 最小长度 (回退模式)
        trna_max_len: tRNA 最大长度 (回退模式)
    ...
    # 关键词匹配太少 → 自动切换为长度+结构回退
    if len(trna_list) < 5:
        print(f"[classify] 关键词匹配仅找到 {len(trna_list)} 条 tRNA (关键词: {trna_keyword})")
        print(f"[classify] 自动切换为长度+结构回退模式 (长度 {trna_min_len}-{trna_max_len}nt, >=15对碱基)")
        
        trna_list = []
        non_trna_list = []
        for entry in entries:
            if is_trna_by_length_and_structure(
                entry,
                min_len=trna_min_len,
                max_len=trna_max_len,
                min_pairs=15,
            ):
                trna_list.append(entry)
            else:
    ...
    # 打印 tRNA 长度分布（诊断用）
    if trna_list:
        trna_lens = [e.length for e in trna_list]
        print(f"[classify] tRNA 长度范围: {min(trna_lens)}-{max(trna_lens)}nt, "
              f"中位数: {np.median(trna_lens):.0f}nt")
  ...
  python pseudo_chimera_generator.py \\
    ...
    parser.add_argument('--tRNA_keyword', type=str, default='tRNA',
                        help='tRNA 识别关键词，多词用逗号分隔 (默认: tRNA)。'
                             '如果用关键词匹配不到，会自动回退到长度+结构识别')
    parser.add_argument('--tRNA_min_len', type=int, default=60,
                        help='tRNA 最小长度（回退模式，默认: 60）')
    parser.add_argument('--tRNA_max_len', type=int, default=95,
                        help='tRNA 最大长度（回退模式，默认: 95）')
    ...
    # 2. 分类 tRNA / 非tRNA
    # ========================================
    trna_entries, non_trna_entries = classify_entries(
        entries,
        trna_keyword=args.tRNA_keyword,
        trna_min_len=args.tRNA_min_len,
        trna_max_len=args.tRNA_max_len,
    )
